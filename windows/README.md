...
**Space は確定ではなく変換である**（世間の IME と同じ作法）。頭脳は核の
[`henkan`](../core/src/henkan.rs) に在り、この殻は駆動するだけで一つも持たない。

| 鍵 | 未変換のとき | 変換中 |
|---|---|---|
| `Space` | **変換する** | 次の候補へ |
| `Enter` | 仮名のまま確定 | 選んだ形で確定 |
| `Esc` | 捨てる | 変換を解いて仮名へ戻す（**読みは失はない**） |
| `BackSpace` | 一字消す | 同上（消さずに解く） |
| `←` `→` | — | 注目する文節を移す |
| `Shift+←` `Shift+→` | — | 注目文節を縮める／伸ばす |
| `↑` `↓` | — | 前の／次の候補へ |

```
  u d g ^y o 2 ^^ o t ^4  Space  Enter   →   今日は良き天氣なり
  u d g ^y o 2 ^^ o t ^4         Enter   →   けふはよきてんきなり
```

**旧字は確定の瞬間だけ掛かる。** 辞書は新字体で持つてゐるので、候補を並べてゐる間の
表記も新字体のままである（「天気」→ 確定して「天氣」）。同じ知識を二か所に置かない
ためで、候補の最後尾には**全部仮名の控へ**が必ず残るので、変換してからでも仮名へ
辿り着ける。

矢印は VK で引く。拡張キーなので走査符号は `0x4B`・`0x48`・`0x4D`・`0x50` であり、
原器が位置で引く三鍵（`0x28`・`0x27`・`0x0D`）とは重ならない——重なれば
「文節を移さうとしたら さ が入る」といふ無音の事故になるので、試験で縛つてある。

`Shift` は `Ctrl`/`Alt` とは別に見る。原器の前置シフトは `^` の**逐次打鍵**であつて
同時押しではないので、`Shift+矢印` を区切り修正に使つても衝突しない。
...
| 項目 | 現況 |
|---|---|
| **候補窓**（`ITfCandidateListUIElement`） | **無い。** 候補は未確定文字列の中で送る（`Space`・`↑↓`）ので打てはするが、**何番目の候補に居るか・他に何が在るかが見えない**。`GUID_TFCAT_TIPCAP_UIELEMENTENABLED` は既に名乗つてある |
| 注目文節の見分け | **無い。** 全体が同じ下線なので、`←→` で移した注目がどこに在るか見えない（`focus_range` は核が返してゐるので、殻が描けば済む） |
| 辞書の厚み | `core/data/jisho.tsv` は **232 語の種**。青空文庫からの収穫（`harvest_jisho.py`）はまだ回してゐない |
